...
decomposition/planning session for CCA-14/CCA-15, or real code-signing certificates for
CCA-10). Run `/backlog-handover init` to start a fresh campaign once new agent-resolvable work
exists (e.g. after CCA-14/CCA-15 are split into subtasks, or certificates are provisioned).
...
- Queue order: CCA-16 before CCA-12.
- CCA-14 and CCA-15 are deliberately EXCLUDED from this campaign. Both explicitly need
  splitting into subtasks before they're wave-sized (their own task descriptions say so), and
  CCA-14 needs a live OpenRouter credential of unknown availability for AC#3/#10. Scoping them
  is a separate planning session, not a campaign wave. Consequently CCA-11 (deps: CCA-15) and
  CCA-13 (deps: CCA-14) cannot become ready inside this campaign either — see Not queued.
- CCA-12's GitHub repo rename (evolvconsulting/nvidia-cowork → claude-conduit) stays a MANUAL
...
## Confirmed at restore #2 (2026-08-01) — wave 2 scope vs CCA-12

CCA-17 and CCA-18 (both created at wave 1 settlement) were unordered relative to CCA-12 in
the confirmed queue. File-citation conflict check found CCA-12 conflicts with BOTH: it
explicitly touches `DESIGN.md` (README/DESIGN.md/CLAUDE.md rebrand updates) and the generated
`licenses.json`, which collide with CCA-17 AC#4 (DESIGN.md §11 update) and CCA-18 (regenerates
licenses.json) respectively. CCA-17 and CCA-18 do not conflict with each other. So CCA-12
cannot share a wave with either regardless of ordering — it was always going to be a solo wave.
User chose (AskUserQuestion): run CCA-17 + CCA-18 as wave 2 first (lower risk, ready now, no
live-verification requirement), defer CCA-12 to its own wave next. Do not re-ask this ordering.

## Confirmed at restore #3 (2026-08-01) — wave 3 scope vs CCA-19

Re-ran the file-citation conflict check fresh rather than trusting the previous handover's
assumption: CCA-19 only touches `test/main/licenses.test.js` (making the tree-coverage
assertion platform-aware), while CCA-12 touches the generated `licenses.json` itself (its
`app.name` field derives from `package.json`'s `productName`, which CCA-12 changes) but not
the test file. **They do not share a file** — unlike CCA-17/18, this pairing was not actually
forced apart by the conflict graph. Presented this finding to the user (AskUserQuestion) with
three options (CCA-12 solo / CCA-19-then-CCA-12 / both in parallel); user chose **CCA-12
...
## Resolved at restore #4 (2026-08-01) — GitHub repo renamed; CCA-9 unblocked in practice

CCA-9 (deps: CCA-12, done in wave 3) was newly unblocked at the Backlog-dependency level, but
...
to the user via AskUserQuestion with three options (defer CCA-9 / dispatch it scoped-down with
AC#5 deferred / run the manual rename now). User chose: **run the manual rename now, then
dispatch CCA-9 in full.** Orchestrator ran `gh repo rename claude-conduit --repo
...
origin`. **The GitHub repo rename is DONE.** CCA-9 entered wave 4 in full (not scoped down).

## Frontier — campaign complete, no ready tasks remain

As of wave 4 settlement (2026-08-01): CCA-19 and CCA-9 are Done (see Resolved). Every
remaining open Backlog task (`backlog task list --exclude-status Done`: CCA-7, CCA-10,
CCA-11, CCA-13, CCA-14, CCA-15) is already accounted for in "Not queued — needs a human /
...
- 2026-08-01 — wave 1 (task: CCA-16): dispatched alone (Shared Machine State cap). Worker
...
- 2026-08-01 — wave 2 (tasks: CCA-17, CCA-18): dispatched in parallel. Both APPROVE from
  independent opus reviewers. Merge queue deliberately reordered (CCA-18 first) to dodge a
  ...
  one non-blocking observation became CCA-19 (user-approved follow-up task).

- 2026-08-01 — wave 3 (task: CCA-12): solo. Review pass 1 request_changes (real reproducible
  ...
  migrated. CCA-9 (deps: CCA-12) unblocked.
...
- 2026-08-01 — restore #4 (pre-wave-4 reconciliation, no code change): the actual GitHub repo
  ...
  local origin updated; connectivity verified. Unblocked CCA-9 in practice.

- 2026-08-01 — wave 4 (tasks: CCA-19, CCA-9): dispatched in parallel (sonnet workers, two
  fresh treehouse worktrees on a newly-cycled pool — cold, required npm install for both).
  CCA-19: opus review APPROVE first pass, all 4 ACs confirmed, 4-mutant mutation testing (3
  real guards, 1 documented gap). CCA-9: opus review pass 1 request_changes — one real MEDIUM
  ...
  would have wrongly discredited an accurate prior task note and misled CCA-10) plus 3 LOW
  ...
  merged: CCA-19 via PR #6 (squash b11e5be), CCA-9 via PR #7 (squash ef793b4), rebased and
  re-tested cleanly in queue order, worktrees released, branches deleted. Wave-level
  integration review (opus) independently re-derived CCA-19's arithmetic with CCA-9's new
  ...
  Housekeeping note: two /tmp directories from CCA-9's live verification
  ...
  **Frontier recomputed after settlement: empty. No remaining open task is agent-resolvable
  without either a human decomposition session (CCA-14/15 and their dependents) or externally
  provisioned code-signing certificates (CCA-10). Campaign complete.**

...
`claude-conduit-app`, tracked by CCA-63; the working copy also moved on disk to
`/Volumes/_data/repos/claude-conduit-app`), which broke all 4 treehouse pool worktrees' back-links to the
main repo (`git worktree repair` fixed them) and appears to have dropped treehouse's own pool registration
entirely (`treehouse status` now reports zero worktrees in the pool, though the 4 directories under
`~/.treehouse/claude-conduit-163fa4/` still physically exist) — treehouse is effectively unavailable until
a fresh `treehouse init`/lease cycle re-establishes it; treat worktree 3 as a manually-managed fallback
worktree for now, not a pool lease. One interrupted Backlog write was recovered and committed (CCA-64,
filed but never `git add`ed by whatever session created it). **CCA-58 and CCA-59 are Done** (task-level
settlement — check-ac/final-summary/-s Done — already happened; only this tracker doc's Queue/Resolved
tables were stale) — moved to Resolved below. **CCA-60 is genuinely mid-flight**: review pass 2 came back
`request_changes` (one blocking claim-accuracy defect, B1, plus should-fix S1/S3), fix pass 2 was
dispatched into worktree 3 to adopt tolerate-and-recover / revert the pre-existing-test edit / remove the
`require.main` fragility, and that fix pass's work is sitting **uncommitted** in the worktree (3 modified
test files + an untracked `test/engine/.helpers/win32-override-scanner.js` extraction), on top of a local
rebase onto `de6c1c3` that already supersedes the 3 commits currently pushed to
`origin/fix/NCOW-60-test-real-windows-config`. Nothing lost; resuming in place. **New tasks discovered that
were never through this campaign's queue-order confirmation**: CCA-62 (evolv hosted gateway
provider/connection type), CCA-63 (GitHub-rename URL sweep, HIGH), CCA-64 (Dependabot js-yaml DoS advisory,
HIGH, just recovered). **A large feature landed directly on `dev`, outside this campaign entirely**: CCA-14.1
and CCA-14.2 (provider abstraction; NVIDIA + OpenRouter) are both already Done, via commit `a1e282e`, with
no PR/worktree/review trail in this tracker — presumably done by the user directly. This means CCA-14's own
"needs decomposition" framing in Not-queued below may be stale; re-check at the next inventory pass, along
with whether CCA-62/63/64 should be added to the confirmed queue order (ask the user before doing so, per
Task-write concurrency). Backlog prefix migration NCOW-N -> CCA-N (numbering preserved) happened between
sessions, confirmed via `92a6be0`; all IDs below already use CCA- (rewritten in place where the underlying
task numbering is unchanged). Plan: finish resuming CCA-60 first (real WIP at risk), then bring this to the
user for a fresh queue-order confirmation before dispatching a further wave.
...
Prior note, as of wave 16 DISPATCH (2026-08-06): ground-truth drift check found `dev` in sync with
...
- 2026-08-07 — **wave 16 settled (task: CCA-57, Done)**: dispatch (conflict graph computed fresh via
  ...
  **What the wave actually found, and it was not what the task was filed as.** CCA-57 was written as
  ...
  from *both fail identically* — and then found the mechanism: `installer.nsh:200`'s
  `WinShell::SetLnkAUMI "${APP_ID}"` stamps `appId` on the shortcut, while the running process used
  Electron's lazy fallback `electron.app.Claude Conduit`. **The mismatch affected nsis exactly as much
  as portable.** The real axis was packaged-vs-Squirrel, not portable-vs-nsis. The user then chose to
  set the AUMID to the appId unconditionally; four values now agree, each read live.

  **Two ACs were amended mid-flight by the user, not quietly reinterpreted by an agent.** Pixel-level
  toast capture proved unobtainable on winvm across four strategies, so AC#1/#3 moved to an
  acceptance-plus-AUMID-correctness standard — the same dispensation AC#4 got for Linux. Recorded on
  the task itself with the reason.

  **The claim-class defect recurred in THREE consecutive passes** (this campaign's failure class 1).
...
- 2026-08-06 — **wave 15 settled (task: CCA-56, Done)**: dispatch (**the first wave in several
  ...
  having corrected a false claim in one file while leaving a verbatim duplicate in another; pass 3
  approved after fix pass 2 ran a claim sweep → serial merge (clean rebase across the
  ...
  **Lessons this wave, both new to the campaign's catalogue:**
  1. **"A correction that fixes an instance rather than the claim."** Fix pass 1 corrected a false
     statement where it was raised and shipped an identical copy elsewhere in the same branch. The
     mitigation now baked into dispatch prompts: whenever a claim is corrected, sweep the whole
     repo for every restatement of it and report the sweep, rather than fixing the cited line.
  2. **"Passes pre-fix" is not evidence a test is a designed control.** Two review passes read a
     test's pre-fix pass as deliberate control design; it passed because it was a verbatim copy of
     a pre-existing test. The check that was run was "does it pass pre-fix"; the check that was
     missing was "is this already covered". A test can also assert a guarantee it does not provide

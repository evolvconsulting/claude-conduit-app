...
requirement. The ToastActivatorCLSID half is tracked separately as CCA-61.
...
Branch `feat/CCA-57-tray-notify-deliverability`, based at `09cbdd98bbeba7ff04c48fad28ee442e783793a2`.
...
appUserModelId.js. Deliberately kept narrow so as not to pre-empt CCA-58.
...
3. On winvm, `taskkill /F /IM "Claude Conduit.exe" /T` (clearing the nsis instance so the
   ...
   the app binary (this project's own CCA-22/24 design). Disposable test VM, no other apps on
...
4. The NSIS uninstaller (/S) reported success but left the whole app directory behind; cleaned up
   manually. Out of CCA-57 scope, not investigated.
...
inherits AC#2's unsupported reading. Merge-order risk: CCA-59 edits the same tray.js docblock
...
credentials; it predates CCA-57 and rotation/removal is the user's call, out of scope here.
...
3. **The pre-existing `C:\Users\jdnewhouse\AppData\Roaming\claude-conduit\litellm.env` on winvm
   (dated 2026-08-02, from an earlier wave's real install, NOT from CCA-57)** — user elected to
   be informed only; no action taken by this campaign. Out of CCA-57's scope either way.
4. Session budget: continue into a wave 17 (CCA-58 + CCA-59) after CCA-57 settles.
...
`paths.resolveWindowsAppDataOverrides()` — exactly the CCA-23 failure class already fixed in
...
**Scope: CLEAN.** README/DESIGN still have zero notification mentions (CCA-58 unpre-empted).
Reviewer ran the esprima token-stream diff on `tray.js` ITSELF: 901 tokens both sides, streams
byte-identical — so CCA-59's tray.js code surface is untouched. electron-builder.yml is
...
**Scope CLEAN.** README/DESIGN/docs untouched (0 files in the diff) — CCA-58 unpre-empted.
...
untouched — CCA-59's code surface intact.
...
worktree's own copy of the CCA-57 task file still carries pre-amendment AC text; the branch touches
...
Tests 476 -> 485, zero pre-existing test files modified. Merged as PR #62 (`97f13aafa080aecdf24305a00f9f39b0e65d5d70`) after 3 review passes and 2 fix cycles, followed by cleanup PR #63 (`903bca54456697d971b1c497b0128fd770a6578e`) closing 6 of the 8 wave-level integration-review findings after 2 further review passes. Two findings became their own tasks: CCA-60 (npm test overwrites the real Windows config dir) and CCA-61 (the ToastActivatorCLSID half of the same Windows requirement, which this task did not close).

...
Non-blocking finding from CCA-22's wave-6 review pass 2 (2026-08-02), accepted deliberately at the time as the lesser of two evils.

CCA-22 fixed a real leak: spawnDaemon() previously never killed the child it spawned on its reject paths, so a persistently-failing bootstrap leaked one Electron-weight pm2 daemon per retry (reproduced live as 3 simultaneous orphans). The fix adds a best-effort child.kill() on every reject path, including the timeout path.
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The timeout path adopts a slow-but-healthy daemon instead of killing it (probe first, treat alive as success), while genuinely failed spawns are still killed so CCA-22's leak fix is preserved
...
unref, resolve {pid: child.pid}) instead of killing it, preserving CCA-22's leak fix for genuine
...
<!-- SECTION:NOTES:BEGIN -->
Wave 9 implementation complete, pushed to fix/CCA-26-adopt-slow-daemon (commits ad4e232, 3739c67).
...
engine-context.js (CCA-23's territory) is the sole production wiring point for spawnDaemon, so
...
preserving CCA-22's leak fix for genuinely failed spawns while letting a merely-slow daemon
...
rebasing onto CCA-23. Squash-merged PR #15 -> dev @ 3ea0fb3.

...
Found independently by both CCA-28's and CCA-29's opus reviewers during wave 11
...
subsequent app upgrade -- including auto-updates via CCA-10's mechanism. Any
fix that lives inside generateAll()'s output (CCA-27's interpreter:
process.execPath + ELECTRON_RUN_AS_NODE, CCA-28's PYTHONIOENCODING=utf-8, and
...
Both v0.1.0 and v0.1.1 (the only two real published releases) predate CCA-27
entirely, so every real user who has ever completed setup against a published
build is currently exposed to this gap.

This intersects with CCA-24 (bootstrapped daemon persistence across
app/proxy lifecycle) and CCA-10's auto-update mechanism -- coordinate scope
...
- [x] #3 Coordinate with CCA-24 if the fix touches the same daemon-lifecycle code, and with CCA-10's auto-update path if regeneration should be triggered by (or verified across) an auto-update rather than only at every app launch
...
6. Verify AC#1/#2 live via NIM_PROXY_TEST_HOME + --dev (never the real config
   dir): seed a fake home with a stale manifest + pre-CCA-27-shaped
...
generated_by_version and a pre-CCA-27-shaped ecosystem.config.cjs (no
...
pushed to origin/fix/CCA-30-regenerate-configs-on-upgrade.
...
(not amended onto 20f84bb), pushed to origin/fix/CCA-30-regenerate-
configs-on-upgrade.
...
diff stays within its claimed findings, both commits carry Refs CCA-30.
...
CCA-27/28-class fixes silently never reached a user who had already
...
and reviewed AC#3 by inspection (pm2Control.js untouched, no CCA-24

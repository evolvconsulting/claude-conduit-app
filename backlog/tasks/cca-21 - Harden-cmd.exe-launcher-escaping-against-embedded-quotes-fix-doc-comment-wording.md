...
Both findings were verified via live testing against the real Windows VM (winvm) during CCA-20's review -- see that task's full notes for the complete evidence trail (opus review pass 3 in particular).
...
- [x] #2 A regression test in test/engine/configGen.test.js covers an arg combining an embedded literal quote AND a cmd.exe metacharacter in the same value (e.g. something shaped like the injection case documented on CCA-20: an arg containing both " and & such that a naive escape would let the & execute)
...
<!-- SECTION:NOTES:BEGIN -->
Implemented on fix/CCA-21-cmd-exe-embedded-quote-escaping (dev @ a7aa4e7), commits 228de79 + caa8681, pushed.

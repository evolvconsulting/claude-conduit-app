...
CCA-56's wave-15 integration review surfaced a defect in `src/main/tray.js`'s notification path that three separate review passes agreed was real but out of CCA-56's own acceptance-criteria scope, so it was deliberately deferred rather than fixed silently.
...
- The class is PRE-EXISTING, not introduced by CCA-56 — the pre-CCA-56 code rejects identically under the same throwing fake, because the `.catch()` limb that produces the behavior is unchanged.
- It is realistically unreachable with Electron's real `Notification`, whose `isSupported()` does not throw.
- CCA-56 DID add a second entry point into it: a resolved `{ok:false}` result now also reaches `notifyFailure()` via the new `.then()` limb, on a path whose throw lands in that same `.catch()`.
...
<!-- SECTION:NOTES:BEGIN -->
## Wave-17 implementation evidence (worker, branch `fix/CCA-59-contain-issupported-throw`, commit `e8c728f494454dc60e5b53ff3275af186863cd9d`, branched from `20ffa60add5d7e281a2f39610adcec1ee987b489`)
...
pre-fix source:
```
not ok 22 - createTrayActions: CCA-59 - a wedged (rejecting) tray Stop with a throwing Notification.isSupported()
  error: onStop() must never reject even when isSupported() throws, but it rejected with: boom from isSupported
not ok 23 - createTrayActions: CCA-59 - a resolved {ok:false} tray Start with a throwing Notification.isSupported()
...
CCA-55 and CCA-56 wedge and resolved-`{ok:false}` tests). All of those use
...
Reviewed `fix/CCA-59-contain-issupported-throw` @ `e8c728f494454dc60e5b53ff3275af186863cd9d` against
...
Hunk header `@@ -341,8 +341,8 @@` — the CCA-57 AUMID comment block at lines 224-290 is entirely
...
`76a7c3c fix(tray): give wedged Start/Stop/Restart a user-visible error surface (CCA-55) (#58)`, and
`git log -S"Notification.isSupported"` shows it was never modified afterward — so the branch's claim
"pre-existing since CCA-55, unchanged by CCA-56" is accurate. Both the commit body and the test
comment state plainly that CCA-56 only added a second entry point.

**ID citation sweep: CLEAN.** CCA-55, CCA-56, CCA-59 only; all resolve to filed tasks. Trailer
`Refs CCA-59.` present (that convention appears in 423 commits). The reviewer explicitly declined to
...
The class is PRE-EXISTING, established here by provenance rather than by reading: `git blame -L 340,346` attributes the guard line to CCA-55's commit `76a7c3c`, and `git log -S"Notification.isSupported"` shows it was never modified afterward. CCA-56 only added a second entry point (a resolved `{ok:false}` now also reaches `notifyFailure()`). It is unreachable with Electron's real `Notification`.
...
Scope verified clean: the CCA-57 AUMID comment block was not edited or reflowed, and the deliberately-unresolved macOS ad-hoc-signing question was not guessed shut.

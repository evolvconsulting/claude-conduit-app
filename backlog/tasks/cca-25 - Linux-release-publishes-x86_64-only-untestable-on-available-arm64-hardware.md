...
Surfaced while selecting a Linux host for CCA-22's cold-bootstrap verification (wave 6, 2026-08-02).
...
Every Linux machine available to this project's maintainer is aarch64 (verified on the tailnet: linuxvm, spark, rpi5, jetson, remote — all aarch64). The practical consequence is that the shipped Linux artifact cannot be run, smoke-tested, or verified on any machine the maintainer owns. CCA-22's Linux verification had to be done from a source checkout built on the host, which means the PACKAGED Linux path has never been exercised at all — the packaged cold-bootstrap was only ever confirmed on macOS.

This is a real gap in the release matrix rather than a cosmetic one: CCA-10.2 established CI publishing for three platforms, and CCA-10.3 verified real end-to-end auto-update on Windows, but the Linux artifact has no verification story on the hardware that exists here.
...
<!-- SECTION:NOTES:BEGIN -->
Wave 9 implementation complete, pushed to feat/CCA-25-linux-arm64-release (4 commits:
...
CCA-22), so it can MODULE_NOT_FOUND on pm2's own ProcessContainerFork.js. The worker
...
install before (CCA-22's Linux verification used a source checkout; its macOS verification
...
ELECTRON_RUN_AS_NODE, established by CCA-22) but nothing equivalent exists for the managed app.
...
why CCA-22 never caught it: every one of its start/stop/restart verifications on all 3 platforms
...
artifact, on any platform, ever. This means the entire CCA-10 auto-update epic's "verified real
...
reference a dead path after the AppImage unmounts - entangles with CCA-24's already-filed daemon-
persistence concerns). Reviewer's own lean: merge CCA-25 now (its own scope is solid and
...
campaign wave exercised a genuinely packaged artifact's cold proxy-start at all. Filed as CCA-27
(High priority) with the reviewer's full root cause and a validated fix recipe. User explicitly
decided (2026-08-03): merge CCA-25 now with this gap documented rather than holding it, since
CCA-27 is unrelated to arm64 specifically and blocks every platform equally. AC#4 not applicable

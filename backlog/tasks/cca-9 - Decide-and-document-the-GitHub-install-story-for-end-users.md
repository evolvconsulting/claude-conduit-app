...
reads (unblocking CCA-10), and package-manager distribution (Homebrew
...
unblocking CCA-10. Also corrected two README inaccuracies: the old
...
1. Now DEPENDS ON CCA-12 (rebrand). Publishing under the current name then renaming would break download links, the appId and any update feed, so the rename lands first.
2. npm run pack and npm run dist were broken at the schema-validation level (linux.desktopName was removed in electron-builder 26) and were fixed under CCA-2. Packaging works again, verified by a real macOS build.
...
4. dist/ already contains latest.yml, latest-mac.yml and latest-linux.yml - electron-builder emits update metadata by default, which CCA-10 needs.

Worker evidence (implementation phase): branch feat/CCA-9-github-install-
...
1. Add a GitHub Actions release workflow (tag push -> build all platforms ->
   ...
   by hand. Prerequisite for CCA-10 being trustworthy.
...
cca-19, git diff dev...HEAD -- backlog/ empty).
...
being emitted -- meaning CCA-9's own prior task note #4 (which the worker's
doc explicitly contradicted) was CORRECT, not stale. CCA-10 is a direct
...
impossible for a metadata+Markdown diff to cause. CCA-19 overlap: confirmed
...
verified the npm-ls arithmetic CCA-19 hardens still holds (79 = 78+1) with
...
feat/CCA-9-github-install-story, pushed. 3 files changed (+60/-24):
...
all along) and explicitly restoring CCA-9's prior task note claiming
latest*.yml already existed as correct, calling out the CCA-10 dependency.
...
npm test: 176/176 pass (branch's own pre-wave-4 baseline; CCA-19's extra
...
settled judgment calls (package.json scope, 170/176 anomaly, CCA-19 merge
safety).
...
CCA-19 interaction reconfirmed: none -- metadata fields add no npm-ls
nodes.
...
CLEARED TO MERGE. Proceeding to the merge queue: CCA-19 first (confirmed
queue order), then CCA-9.
<!-- SECTION:NOTES:END -->

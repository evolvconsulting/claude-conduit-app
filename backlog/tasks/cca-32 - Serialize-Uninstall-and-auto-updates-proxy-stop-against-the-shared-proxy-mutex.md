...
7. Add 5 regression tests to test/main/ipc-mutex.test.js reusing the existing fake-electron
   require.cache shim and held-mutex pattern from CCA-31's own tests.
...
in both CCA-17's and this task's comments) -- overlapping diagnostic runs are only
prevented by the renderer disabling its own button, not at the IPC layer.

Branch fix/CCA-32-serialize-uninstall-update-proxy-mutex pushed to origin. Two commits:
fix(ipc): serialize uninstall and update-install against the proxy mutex;
test(ipc): cover CCA-32's uninstall/update-install mutex aliasing.
...
createEngineContext(), before the window is interactive, and outside CCA-32's own ACs;
flagged for a future backlog item, not this branch. No overlap risk with CCA-44 (disjoint

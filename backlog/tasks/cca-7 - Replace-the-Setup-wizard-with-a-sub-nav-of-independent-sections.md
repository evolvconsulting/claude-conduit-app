...
<!-- SECTION:NOTES:BEGIN -->
PARKED pending CCA-15 (user decision, 2026-07-31). CCA-13 moves Prerequisites out of Setup into System Settings, and CCA-15 turns per-connection configuration into a connection library, so rebuilding the wizard structure now would very likely be thrown away. The sub-nav requirement is not dropped - it should be satisfied as part of CCA-15, where the connection model determines the structure. Revisit this task after CCA-15 and either close it as covered or scope whatever remains.
<!-- SECTION:NOTES:END -->

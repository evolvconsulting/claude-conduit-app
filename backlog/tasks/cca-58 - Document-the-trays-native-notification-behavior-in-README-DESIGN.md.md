...
- [x] #3 Known platform caveats (Windows AUMID/portable-build gap, macOS DND/permission-denied) are mentioned or linked to CCA-57's resolution, whichever is accurate at the time this task is done
...
- [x] #5 The deliberate tray-Start-vs-dashboard-#start-btn asymmetry from CCA-56's AC#2 decision is documented where a user can read it, including why tray Start stays enabled with no manifest and notifies on click instead of being disabled
- [x] #6 README.md:331's `npm test` comment no longer asserts a protection that does not hold: the "no real config touched" half is corrected or dropped so the line is accurate on Windows as well, and accurate regardless of whether CCA-60 lands (do NOT write a claim whose truth depends on a sibling task merging)
<!-- AC:END -->
...
2. Read the ACTUAL current source rather than trusting the task description's summary (the task warns the description predates some of that code): src/main/tray.js (full createTray/createTrayActions including the CCA-55/56/57 comment blocks), src/main/appUserModelId.js, electron-builder.yml's win.target comment, and src/renderer/views/dashboard-view.js to confirm #start-btn's disabled expression.
...
CCA-60 establishes that half is FALSE on Windows (`npm test` overwrites the real
`%APPDATA%\claude-conduit`). The user was offered three homes for the correction — this task, CCA-60,
or leave it as a note — and chose this task, because this task is the only wave-17 member editing
README.md, which keeps README single-owner for the wave and lets CCA-58/59/60 run genuinely parallel.

Deliberate constraint on the wording, and the reason AC#6 says it explicitly: the corrected line must be
true on its own, not conditional on CCA-60 merging. Dropping or qualifying the false half satisfies
this; re-asserting the protection because a sibling branch is expected to land does not.

## Wave-17 implementation evidence (worker, branch `docs/CCA-58-tray-notify-docs`, commit `16c35195ff3e5b771f16f9ef29c0d926401569ae`, branched from `20ffa60add5d7e281a2f39610adcec1ee987b489`)
...
**AC#3** — wrote a "Known platform caveats (CCA-57)" bullet list covering Windows (AUMID now matches
...
`dbus-monitor` against GNOME), plus the always-on console-log fallback. CCA-57 is treated as LANDED
...
v43.2.0. It paraphrased only what `src/main/tray.js`'s CCA-57 comment block and
...
`                       # the real %APPDATA%\claude-conduit - tracked as CCA-60)`
That makes a PRESENT-TENSE behavioral claim whose truth value inverts the moment CCA-60's fix lands -
which is expected in this same wave. AC#6 required wording accurate regardless of whether CCA-60
lands; this inverts the dependency rather than removing it (false-after-merge instead of the
false-today-true-after-merge failure the AC's parenthetical guarded against). The `CCA-60` citation
...
That same line asserts `485 tests`. Wave 17's sibling branches add tests (CCA-59 adds 2; CCA-60 adds
...
Reviewed `docs/CCA-58-tray-notify-docs` @ `16c35195ff3e5b771f16f9ef29c0d926401569ae` against wave base
...
- **B1 (DESIGN.md:422-424) — false counterfactual.** "Before CCA-55, a thrown/rejected
  `handlers.proxy.*()` call reached only `console.error`" is true of `onStop` ONLY, and only since
  CCA-53. The reviewer read the pre-CCA-55 source directly (`git show 76a7c3c^:src/main/tray.js`
  ...
  and calls it "worse than onStop's pre-CCA-53 silence"; the new prose flattens it and understates the
  pre-fix state.
...
Fixing it now also makes the section survive either CCA-61 resolution and removes a doc edit from that
task's plate, which CCA-61's own text anticipates.

**N1-N3 (nits)**: DESIGN.md:430 says CCA-57 "tracked by" these caveats when it resolved them (and the
open CLSID half belongs to CCA-61); a one-column comment misalignment at README.md:375; Linux listed
under "Known platform caveats" though its bullet is a positive verification.

**ID citation sweep: CLEAN.** CCA-55 (x3), CCA-56 (x3), CCA-57 (x2), CCA-60 (x1) — all resolve to
real filed tasks. Trailer `Refs CCA-58.` correct. No fabricated IDs.
...
CCA-53) while `onStart`/`onRestart` had none. Rewrote the DESIGN.md sentence to say exactly that,
...
This drops the CCA-60-coupled `%APPDATA%` claim AND the false "no network access" claim, asserting no
...
`portable` only. CCA-61 cited after confirming it is real and open by listing `backlog/tasks/`.

**N1 fixed** ("tracked by CCA-57" -> "documented by CCA-57"). **N2** resolved as a side effect.
...
in-repo. But the chain is not paraphrase-of-a-paraphrase only: `backlog/tasks/cca-61...md:19` records the
near-verbatim statement WITH the version pin, and `cca-57...md:17` states the same with its verification
...
**ID citation sweep CLEAN**: CCA-53, CCA-55, CCA-56, CCA-57 (all Done) and CCA-61 (To Do, correctly
described as "open"). The removed CCA-60 citation is gone from the tree. No abbreviated SHA leaked into
...
specifically fixed and live-verified by CCA-23, whereas this task's defect is in a test-harness path.
Different mechanism, and the claim holds for what it describes.

**Remaining nits, none requiring a pass**: `DESIGN.md:424-425`'s "worse than onStop's pre-CCA-53 silence"
...
**Overlap notes.** CCA-59's prose constraint is satisfied — README never mentions `isSupported()`
throwing and never describes a double-logged or misattributed line, and `README.md:276`/`:310-312` stay
true after CCA-59 lands. The CCA-60 semantic collision is fully dissolved, since this branch now makes
no config-safety claim at all. The wording survives either CCA-61 resolution. **One item for the wave's
post-merge cleanup**: after CCA-59 (+2) and CCA-60 (+2) merge, both `README.md:381` and `CLAUDE.md:51`
...
AC#6 corrected `README.md`'s `npm test` comment, which claimed "no network or real config touched". BOTH halves were false: the config half on Windows (which CCA-60 fixes), and the network half everywhere, because `test/engine/nvidiaKey.test.js:83` skips only when `CI` is set. The line now asserts no protection at all.

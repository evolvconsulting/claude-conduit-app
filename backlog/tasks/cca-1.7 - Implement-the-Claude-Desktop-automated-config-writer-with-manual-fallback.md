...
7. Unit tests with node:test against a temp configLibrary directory fixture (never the real one, which does not exist on this machine anyway per CCA-1.6) - cover: refuses without consent, refuses when dir missing, never touches other entries, merge-preserves-unknown-keys on re-apply, revert creates Claude API entry when none exists, revert reuses an existing anthropic entry when one does exist, backup+restore.

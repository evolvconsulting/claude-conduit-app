...
7. DESIGN.md: update section 6.1's config.yaml sample + comments, section 8's Desktop instructions table + verify text, section 9.1's env-keys table, section 11's test-mode request/sample-output/small-model row, section 12.2 gotcha #2, section 13's T7 manual-test row. Add a short note (matching the section 7.4/NCOW-4 pattern) documenting the NCOW-8 rename and why (nim-large/nim-small are not valid client-facing model IDs).

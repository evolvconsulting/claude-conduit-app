...
CCA-31 deliberately excluded shutdown.js's before-quit proxy stop from the new proxy mutex (queueing it behind a background restart that can hold the lock for up to ~60s+ would risk making the app unquittable while wedged, which CLAUDE.md forbids outright). This was reviewed and accepted as the correct call across two review passes, and is documented in engine-context.js's own code comment (see also CCA-33 for a comment-accuracy correction), but README.md and DESIGN.md section 7.4 -- which already document related pm2/shutdown/quit behavior in detail -- do not yet mention this carve-out at all.
...
5. Commit (docs(shutdown): ...) and push feat/CCA-34-document-shutdown-mutex-carveout.
<!-- SECTION:PLAN:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Worker implementation complete on branch feat/CCA-34-document-shutdown-mutex-carveout (commit 850bf5c), pushed to origin.
